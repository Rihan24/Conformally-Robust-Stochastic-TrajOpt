...
class U_FUNC(nn.Module):
    ...
    def __init__(self, model_u_w1, model_u_w2, num_dim_x, num_dim_control):
        ...
        # self.u_min=torch.tensor([-1.5,-6,-6]).view(1, num_dim_control, 1)
        # self.u_max = torch.tensor([1.5,6,6]).view(1, num_dim_control, 1)
        # self.register_buffer('u_min', torch.tensor([-1.5,-6,-6]).view(1, num_dim_control, 1))
        # self.register_buffer('u_max', torch.tensor([1.5,6,6]).view(1, num_dim_control, 1))
    ...
    def forward(self, x, xe, uref):
        ...
        # scale = (self.u_max - self.u_min) / 2.0  # half the interval length
        # offset = (self.u_max + self.u_min) / 2.0   # center of the interval
        # # Smoothly map u into [u_min, u_max]:
        # u = offset + scale * torch.tanh((u - offset) / scale)
