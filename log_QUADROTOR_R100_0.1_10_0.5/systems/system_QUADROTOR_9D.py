...
def f_func(x):
    ...
    f[:, 3, 0] =     -x7 * torch.cos(x9) * torch.sin(x8) # -x7 * torch.sin(x9)
    f[:, 4, 0] =   x7 * torch.sin(x9)    #x7 * torch.cos(x9) * torch.sin(x8)
    f[:, 5, 0] =    -g + x7 * torch.cos(x9) * torch.cos(x8)  #g - x7 * torch.cos(x9) * torch.cos(x8)
