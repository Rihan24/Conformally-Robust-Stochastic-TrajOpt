...
def forward(x, xref, uref, _lambda, verbose=False, acc=False, detach=False):
    ...
    C_eig, _ = torch.linalg.eig(Contraction)
    C1_eig, _ = torch.linalg.eig(C1_LHS_1)

    C_eig = torch.real(C_eig)
    C1_eig = torch.real(C1_eig)

    # print(Contraction.shape, C_eig.shape)
    # print(((C_eig >= 0).sum(dim=1) == 0).cpu().detach().numpy())
    ...
    if verbose:
        ...
        print(C_eig.min(dim=1)[0].mean(), C_eig.max(dim=1)[0].mean(), C_eig.mean())
    if acc:
        return (
            loss,
            ((C_eig >= 0).sum(dim=1) == 0).cpu().detach().numpy(),
            ((C1_eig >= 0).sum(dim=1) == 0).cpu().detach().numpy(),
...
for epoch in range(args.epochs):
    ...
    if p1+p2 >= best_acc:
        ...
        torch.save({'model_W': model_W.state_dict(), 'model_Wbot': model_Wbot.state_dict(), 'model_u_w1': model_u_w1.state_dict(), 'model_u_w2': model_u_w2.state_dict()}, filename)
