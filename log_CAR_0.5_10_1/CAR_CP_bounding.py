...
controller = get_controller_wrapper('log_CAR/controller_best.pth.tar')      # 1<=CAR2<=2   0.1<=CAR3<=0.2 
w_func = get_w_func_wrapper_from_checkpoint(checkpoint_path='log_CAR/model_best.pth.tar', w_lb=0.1, task = "CAR")
w_lb=0.1
w_ub=10
...
if __name__ == '__main__':
    ...
    for i in range(num_realizations):
        ...
        xinit = xstar_0[0] + xe_0.reshape(-1,1)
        ...
        x, u, omega_CP_sumterm = EulerIntegrate(controller, f, B, xstar,ustar,xinit,w_func,time_bound,time_step,gamma, with_tracking=True,sigma=0.01)
        x=np.array(x)   #[:-1,:,:]
        u=np.array(u)    #[:-1,:,:]
        # print(u)
        lyapunnov_CP_term= compute_lyapunov_modelling_error(x[:,:,0],xstar[:,:,0],w_func,w_lb,w_ub,gamma)
        ...
        s  = lyapunnov_CP_term + np.sqrt(1/w_lb)*omega_CP_sumterm
